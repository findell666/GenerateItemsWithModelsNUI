...
void ModifyCreaturePart(string sModel, string sPartCategory = "")
{   
    int nPart = GetPartIndex(sPartCategory == "" ? GetPartCategorySelected() : sPartCategory);
...
void ModifyArmorPart(string sModel = "", string sPartCategory = "")
{
    ...
    {
        ModifyCreaturePart(sModel, sPartCategory);
    }
...
{
    NUI_DestroyForm(OBJECT_SELF, GetFormToken());
    NUI_DisplayForm(OBJECT_SELF, "appedit_loader");
}
...
void InitializeDatabase(int nType = -1)
{
    ...
    sQuery = "CREATE TABLE IF NOT EXISTS " + DATABASE_TABLE_BASE_ITEM + " (" +
...
{
    string sTable, sTables = DATABASE_TABLE_SIMPLE + "," + DATABASE_TABLE_COMPOSITE + "," + DATABASE_TABLE_ARMOR + "," + DATABASE_TABLE_BASE_ITEM;
        ...
        for (n = 0; n < nCount; n++)
        {
            if (sReturnColumn == TWODA_INDEX)
            {
                ...
                if (HasListItem(sCriteria, sResult) == TRUE)
                    sReturn = AddListItem(sReturn, IntToString(n), TRUE);
            }
            else
            {
...
void AddBaseItemToDatabase(int nIndex, string sLabel, string sClass){
    string sQuery = "INSERT OR REPLACE INTO " + DATABASE_TABLE_BASE_ITEM + " (id, label, class) " + "VALUES (@id, @label, @class);";
    sql = PrepareQuery(sQuery);
    SqlBindInt(sql, "@id", nIndex);
    SqlBindString(sql, "@label", sLabel);
    SqlBindString(sql, "@class", sClass);
    SqlStep(sql);
}


void SortBaseItemsTableByLabel(){
    string sQuery = "WITH OrderedRows AS (SELECT id, ROW_NUMBER() OVER (ORDER BY label ASC) AS id_sorting " +
    " FROM " + DATABASE_TABLE_BASE_ITEM + ") " +
    " UPDATE " + DATABASE_TABLE_BASE_ITEM + 
    " SET id_sorting = (SELECT id_sorting FROM OrderedRows WHERE OrderedRows.id = " + DATABASE_TABLE_BASE_ITEM + ".id)";


WriteTimestampedLogEntry(sQuery);
    sql = PrepareQuery(sQuery);
    int retVal = SqlStep(sql);
    WriteTimestampedLogEntry(SqlGetError(sql));
    WriteTimestampedLogEntry("SortBaseItemsTableByLabel: " + IntToString(retVal));


    // sQuery ="UPDATE "+DATABASE_TABLE_BASE_ITEM+" SET pk = new_pk";
}

void PopulateCompositeModelBaseItemsData()
{
    string sTypes = "2";
    int t, tCount = CountList(sTypes);
    for (t = 0; t < tCount; t++)
    {
        string sType = GetListItem(sTypes, t);
        string sClasses = Get2DAListByCriteria("baseitems", "itemclass", "modeltype", sType);
        string sLabels = Get2DAListByCriteria("baseitems", "label", "modeltype", sType);
        int c, nClasses = CountList(sClasses);
        ...
        {
            string sClass = GetListItem(sClasses, c);
            string sLabel = GetListItem(sLabels, c);
            int nIndex = c;
            WriteTimestampedLogEntry("Adding base item to database: " + IntToString(nIndex)+ " " + sLabel + " (" + sClass + ")");
            DelayCommand(0.1, AddBaseItemToDatabase(nIndex+1, sLabel, sClass));
        }
    }

    SortBaseItemsTableByLabel();
...
void FillBaseItemCombobox(){

    string comboList = "Choose an item type";

    sQuery = "SELECT label, class FROM " + DATABASE_TABLE_BASE_ITEM; 
    sql = NUI_PrepareQuery(sQuery, USE_CAMPAIGN_DATABASE, CAMPAIGN_DATABASE);
    int retVal = SqlStep(sql);
    while(retVal){
        string sLabel = SqlGetString(sql, 0);
        string sClass = SqlGetString(sql, 1);
        comboList += "," + sLabel + " - " + sClass;
        retVal = SqlStep(sql);
    }
    NUI_AddComboboxEntryList(comboList);

}

void CreateComboBaseItemForm(){

    string sFormID = "combo_baseitem";
    NUI_CreateForm(sFormID);
    {
        NUI_SetHeight(120.0);
        NUI_SetScrollbars(NUI_SCROLLBARS_NONE);
        NUI_AddCombobox("baseitem");
            NUI_SetWidth(260.0);
            FillBaseItemCombobox();
            NUI_BindValue("handler_baseitem_value");

    } NUI_SaveForm();

}

string GetSelectedBaseItemColumn(int id, string sColumn){
    string sQuery = "SELECT "+ sColumn+" FROM " + DATABASE_TABLE_BASE_ITEM + " WHERE id = @id;";

    WriteTimestampedLogEntry("GetSelectedBaseItemColumn: " + sQuery);
    sql = PrepareQuery(sQuery);
    SqlBindInt(sql, "@id", id);
    int retVal = SqlStep(sql);

    WriteTimestampedLogEntry("GetSelectedBaseItemColumn: " + IntToString(retVal));
    if(retVal){
        string sColumnValue = SqlGetString(sql, 0);
        return sColumnValue;
    }
    return "";
}
...
void NUI_HandleFormDefinition()
{
        ...
        NUI_SetResizable(TRUE);
    {
        ...
        NUI_AddRow();
            ...
            NUI_AddRow();
                NUI_AddCommandButton("open_form:generator");
                NUI_SetWidth(300.0);
                NUI_SetHeight(60.0);
                NUI_SetLabel("Open Item Generator");

    } NUI_SaveForm();

    sFormID = "appearance_generator";
    NUI_CreateForm(sFormID);
        NUI_SetGeometry(-1.0, -1.0, 330.0, 300.0);
        NUI_SetTitle("Item generator");
        NUI_SetResizable(TRUE);
    {
        NUI_AddControlGroup();
            NUI_SetScrollbars(NUI_SCROLLBARS_NONE);
            NUI_SetHeight(180.0);
            NUI_SetID("combo_baseitem_anchor");
        {
            NUI_AddSpacer();
        } NUI_CloseControlGroup();

        NUI_AddRow();
            NUI_AddCommandButton("generate_items");
            NUI_SetWidth(300.0);
            NUI_SetHeight(60.0);
            NUI_SetLabel("Generate items");
...
void UpdateBinds(string sBind, int nToken = -1, int bSetDefaults = FALSE)
{
    ...
    if (sBind == "geometry_handler")
        jReturn = NUI_DefineRectangle(-1.0, -1.0, 350.0, 620.0);
...
object GenerateItemsInStore(object oPC, string sClass, int isLinked){

    object oStore = CreateObject(OBJECT_TYPE_STORE, BLUE_PRINT_STORE, GetLocation(oPC), FALSE, sClass + "_store_"+IntToString(isLinked));

    string itemResRef = "nw_"+sClass+"001";
    object oItem = CreateItemOnObject(itemResRef, oStore);

    if(!GetIsObjectValid(oItem)){
        WriteTimestampedLogEntry("Failed to create item: " + itemResRef);
        return OBJECT_INVALID;
    }

    json jItem = ObjectToJson(oItem);

    // Get the model part numbers from the database
    string sQuery = "SELECT DISTINCT shape, color FROM " + DATABASE_TABLE_COMPOSITE + " WHERE class = @class ORDER BY file ASC";
    sql = PrepareQuery(sQuery); 
    SqlBindString(sql, "@class", sClass);
    int retVal = SqlStep(sql);
    while(retVal){
        int shape = SqlGetInt(sql, 0);
        int color = SqlGetInt(sql, 1);

        string merged = IntToString(shape)+IntToString(color);
        int linkedModelValue = StringToInt(merged);

        if(0 == linkedModelValue || "" == merged){
            retVal = SqlStep(sql);
            continue;
        }

        int modelBottomPart = linkedModelValue;
        int modelMiddlePart = linkedModelValue;
        int modelTopPart = linkedModelValue;
        ...
        jItem = GffReplaceLocString(jItem, "LocalizedName", itemResRef+"_b"+IntToString(modelBottomPart)+"_m"+ IntToString(modelMiddlePart) +"_t"+ IntToString(modelTopPart));     // Top part

        object oModifiedItem = JsonToObject(jItem, GetLocation(oStore), oStore, FALSE);

        retVal = SqlStep(sql);
    }

    // Destroy the original unmodified item
    DestroyObject(oItem);
...
void NUI_HandleFormEvents()
{
    ...
    if (ed.sFormID == "appearance_generator")
    {

        if (ed.sEvent == "click")
        {

            string element = ed.sControlID;

            if (element == "generate_items"){
                int id = JsonGetInt(NUI_GetBindValue(ed.oPC, ed.nFormToken, "handler_baseitem_value"));
                string sClass = GetSelectedBaseItemColumn(id, "class");
                int isLinked = 1;
                GenerateItemsInStore(ed.oPC, sClass, isLinked);
            }
        }
    }
        ...
        if (ed.sEvent == "click")
        {
            ...
            else if (sFunction == "handler_load")
            {
                if (sType == "all")
                {
                    ...
                    //Populate Composide Model base Items
                    DelayCommand(0.1, PopulateCompositeModelBaseItemsData());
            ...
            else if (sFunction == "open_form")
            {
                ...
                else if (sValue == "generator"){
                    NUI_DestroyForm(ed.oPC, ed.nFormToken);
                    CreateComboBaseItemForm();
                    int nToken = NUI_DisplayForm(ed.oPC, "appearance_generator");                    
                    json jComboBaseItems = NUI_GetFormRoot("combo_baseitem");
                    NuiSetGroupLayout(ed.oPC, nToken, "combo_baseitem_anchor", jComboBaseItems);
                }
            }
        }
    ...
    else if (ed.sEvent == "mouseup")
    {
        ...
        {  
            ...
            OnLinkItemModels();
        else if (ed.sControlID == "open_loader")
            open_loader_click();
        else if (ed.sControlID == "setTargetObject")
            setTargetObject_click();
        else if (ed.sControlID == "cb_self")
            cb_self_mouseup();
    }
    else if (ed.sEvent == "close")
    {
        if (ed.sFormID == FORM_ID)
            form_close();
    }
    else if (ed.sEvent == "open")
        form_open();
